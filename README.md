...
Repo cho bài lab **Multi-Agent Systems**: hệ thống nghiên cứu gồm **Supervisor + Researcher + Analyst + Writer** và benchmark với single-agent baseline.

Repo hiện đã có reference implementation bao gồm guardrails tối thiểu, tracing, và benchmark theo rubric.
...
Lệnh này chạy baseline (single-agent) để so sánh với multi-agent.
...
Lệnh này chạy multi-agent và lưu trace JSON vào `reports/last_trace.json`.
...
- Thay tracing JSON hiện tại bằng LangSmith/Langfuse/OpenTelemetry.
- Thêm đánh giá chất lượng bằng LLM-judge (nếu chấp nhận chi phí).
- Thêm caching cho search/LLM để giảm latency.
